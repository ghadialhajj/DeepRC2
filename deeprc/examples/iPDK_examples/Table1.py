...
hyperparam_names = {'FAE': "mul_att_by_factor", 'AP': 'None', 'FE': "factor_as_attention", 'TE': 'pos_weight',
                    'Vanilla': 'l2_lambda', 'Add': 'l2_lambda', 'Conj': 'l2_lambda'}
...
for fold in range(len(folds)):
    ...
    for hyperparams_val in hyperparams_values[hyperparam_name]:
        ...
        run = wandb.init(project="HIV - Conjunctive", group=f"{config['strategy']}_", reinit=True, config=config, )
        ...
        model = DeepRC(max_seq_len=30, sequence_embedding_network=sequence_embedding_network,
                       ...
                       additive=config['strategy'] == "Add", conjunctive=config['strategy'] == "Conj",
