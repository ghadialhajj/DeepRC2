...
config = {"sequence_reduction_fraction": 0.1, "reduction_mb_size": int(5e3),
          ...
          "dataset": "n_600_wr_0.050%_po_20%", "pos_weight": 100,
          "plain_DeepRC": False, "Branch": "AdHoc1"}  # , "tag": ["AdHoc1.3.1"]}

strategies = ["TE", "TASTE", "T-SAFTE", "FG"]


# TE: Train Everthing
# TASTE: Train Attention and Sequence Embedding first, then Train Everything
# T-SAFTE: Train Attention and Sequence Embedding first, then Freeze the first part and Train Everything
# FG: Forced Guidance: attention is provided rather than learned (regarded of label trueness)
...
for strategy in strategies:
    ...
    run = wandb.init(project="Benchmarking1", group=group)  # , tags=config["tag"])
