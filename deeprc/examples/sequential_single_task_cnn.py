...
parser.add_argument('--n_runs', help='Number of runs to perform. Default: 1.',
                    type=int, default=1)
...
device_name = "cuda:1"  # + str(int((args.ideal + args.ideal) % 2))
...
#           "dataset": "n_reps_5000_obs_prop_1_po_0.010%_puo_0", "run": f"run_{args.ideal}"}
...
# datasets = ["n_600_op_1_po_0.001%_pu_0", "n_600_op_1_po_0.010%_pu_0", "n_600_op_1_po_0.100%_pu_0",
#             "n_600_op_1_po_1.000%_pu_0"]
datasets = ["n_600_op_1_po_0.010%_pu_0"]


def run(trainingset, trainingset_eval, validationset_eval, testset_eval):
    #
    ...
    model = DeepRC(max_seq_len=30, sequence_embedding_network=sequence_embedding_network,
                   ...
                   reduction_mb_size=config["reduction_mb_size"], device=device, ideal=bool(args.ideal)).to(
        device=device)
    #
    # Train DeepRC model
    #
...
for dataset in datasets:
    ...
    trainingset, trainingset_eval, validationset_eval, testset_eval = make_dataloaders(
        ...
        metadata_file=f"{root_dir}/datasets/{dataset}/metadata.tsv",
        n_worker_processes=8,
        repertoiresdata_path=f"{root_dir}/datasets/{dataset}/repertoires",
    ...
    for this_run in range(args.n_runs):
        config = {"sequence_reduction_fraction": 0.1, "reduction_mb_size": int(5e3),
                  "timestamp": datetime.datetime.now().strftime('%Y_%m_%d_%H_%M_%S'),
                  "dataset": dataset, "run": f"run_{args.ideal}"}
        # Append current timestamp to results directory
        results_dir = os.path.join(f"{base_results_dir}_{config['dataset']}", config["timestamp"])
        wandb_run = wandb.init(project="Find_owr", group=config['run'], reinit=True)
        wandb_run.name = config["run"] + f"_idx_{str(this_run)}"  # += f"_ideal_{config['ideal']}"

        wandb.config.update(args)
        wandb.config.update(config)
        with wandb_run:
            run(trainingset, trainingset_eval, validationset_eval, testset_eval)
