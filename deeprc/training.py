...
def train(model: torch.nn.Module, task_definition: TaskDefinition, early_stopping_target_id: str,
          ...
          ignore_missing_target_values: bool = True, idx: int = 0, fold: int = 0):
    ...
    results_directory = os.path.join(results_directory, f'fold{fold}',
                                     f'idx{idx}')  # datetime.datetime.now().strftime('%Y_%m_%d_%H_%M_%S'))
        ...
        try:
            ...
            while update < n_updates:
                for data in trainingset_dataloader:
                    # Get samples as lists
                    labels, inputs, sequence_lengths, counts_per_sequence, sample_ids, weights = data
