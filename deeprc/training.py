    ...
    try:
        ...
        try:
            ...
            while update < n_updates:
                ...
                if second_phase:
                    data = next(iter(trainingset_dataloader))
                ...
                if second_phase:
                    ...
                    loss = l1reg_loss * l1_weight_decay + attention_loss * (not rep_loss_only)
                ...
                if update % evaluate_at == 0 or update == n_updates or update == 1:
                    ...
                    scores, sequence_scores = evaluate(model=model, dataloader=validationset_eval_dataloader,
                                                       task_definition=task_definition, device=device, bag_level=second_phase)
                    if second_phase:
