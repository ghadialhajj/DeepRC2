...
class Logger():
    ...
    def log_attention(self, dl_name, split_attentions, step):
        self.plot_histogram(pos_vals=split_attentions[0],
                            neg_vals=split_attentions[1], dl_name=dl_name,
                            plot_title=f"Positive (B) and Negative (R) Raw Att", xaxis_title="Attention value",
                            step=step)

    def log_logits(self, dl_name, split_logits, step):
        self.plot_histogram(pos_vals=split_logits[0],
                            neg_vals=split_logits[1], dl_name=dl_name,
    ...
    def get_values_per_dl(self, model: torch.nn.Module, dataloader: torch.utils.data.DataLoader,
        ...
        """
        all_logits, all_targets, all_attentions, all_seq_targets, all_emb_reps = get_outputs(model=model,
                                                                                             dataloader=dataloader,
                                                                                             device=device,
                                                                                             show_progress=show_progress)
        split_logits = self.split_outputs(all_logits, all_targets)
        split_attentions = self.split_outputs(all_attentions, all_seq_targets)
    ...
    @staticmethod
    def split_outputs(all_values, all_targets, flatten: bool = True):
        """
...
def get_outputs(model: torch.nn.Module, dataloader: torch.utils.data.DataLoader,
    ...
    return all_logits, all_targets, all_attentions, all_seq_targets, all_emb_reps
